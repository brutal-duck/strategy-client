...
export default class MatchOverBtn {
  public scene: Modal
  public x: number
  public y: number
  ...
  constructor(scene: Modal, x: number, y: number) {
    this.scene = scene
    this.x = x
    this.y = y
    this.init()
  ...
  public create(): void {
    this.border = this.scene.add.sprite(this.x, this.y, 'border-1').setTint(0x909090).setScale(1.5, 1.3).setDepth(2).setInteractive()
    ...
    this.text = this.scene.add.text(this.border.getCenter().x, this.border.getCenter().y, this.scene.lang.continue, { font: '18px Molot', color: 'black' }).setOrigin(0.5)
